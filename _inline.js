...
// ---- Filters (Categoria + Comunità) ----
var FiltersControl = L.Control.extend({
  options: { position: 'topright' },
  onAdd: function() {
    var container = L.DomUtil.create('div', 'leaflet-bar filters-control');
    ...
    var wrap = L.DomUtil.create('div', '', container);
    wrap.style.display = 'flex';
    wrap.style.gap = '18px';
    wrap.style.alignItems = 'flex-start';

    // --- Column builders ---
    function makeColumn(titleText) {
      var col = L.DomUtil.create('div', '', wrap);
      col.style.flex = '1';
      col.style.minWidth = '320px';

      var title = L.DomUtil.create('div', '', col);
    ...
    function makeRow(parent, labelText, iconHtml, checked, onChange) {
      var row = L.DomUtil.create('label', 'cat-row', parent);
      ...
      var icon = L.DomUtil.create('span', 'filter-icon', row);
    ...
    // ===== Categoria =====
    var catCol = makeColumn('Categoria');
    var catCbs = [];
    // Symbolic black/white icons for categories (SVG, no colour swatches)
    var CATEGORY_SVGS = {
    ...
    // "Tutte" category toggle
    var allCatCb = makeRow(catCol, 'Tutte', CATEGORY_SVGS['all'], true, function(){
    ...
    for (var i = 0; i < categories.length; i++) {
      (function(catName) {
        var cb = makeRow(catCol, catName, catIcon(catName), true, function() {
          applyFromCheckboxes();
        });
        catCbs.push(cb);
      })(categories[i]);
    ...
    function applyFromCheckboxes() {
      ...
      allCatCb.checked = (selectedCats.length === categories.length);

      // Affiliations are handled by the Comunità buttons (single-select or all)
      rebuildOverlaysAndSearch();
    }

    // ===== Comunità =====
    var affCol = makeColumn('Comunità');
    // Keep the Comunità menu visually stable even with many options
    affCol.style.maxHeight = '320px';
    affCol.style.overflowY = 'auto';
    affCol.style.paddingRight = '4px';

    // Single-select buttons (radio-like): click one -> only that one active.
    // Use a deterministic colour per community for OUTLINE highlighting on the map.
    var affButtons = [];

    function makeAffButton(label, keyLower, isAll) {
      var btn = L.DomUtil.create('button', 'aff-btn', affCol);
      btn.type = 'button';
      btn.setAttribute('data-aff', isAll ? '__all__' : keyLower);
      btn.setAttribute('aria-pressed', 'false');

      var strokeCol = isAll ? '#111' : (AFF_OUTLINE_COLORS[keyLower] || '#111');
      btn.innerHTML =
      ...
      btn.addEventListener('click', function(e) {
        ...
        // Comunità selection MUST NOT filter or rebuild layers.
        // It only toggles outline highlighting while time continues.
        activeAffiliationsLower = affiliationsLowerAll.slice(); // keep "all" for compatibility
        ...
        // Keep a stable order (so ring layering doesn't jump around)
        try {
    ...
    function updateAffButtons() {
      affButtons.forEach(function(btn) {
        var k = btn.getAttribute('data-aff');
        var active = false;
        if (k === '__all__') {
          active = (selectedAffiliationsLower.length === 0);
        } else {
          active = (selectedAffiliationsLower.indexOf(k) !== -1);
        }
        if (active) btn.classList.add('is-active');
        else btn.classList.remove('is-active');
        try { btn.setAttribute('aria-pressed', active ? 'true' : 'false'); } catch(e) {}
      });
    }

    // Build UI
    makeAffButton('Tutte', null, true);
    for (var k = 0; k < affiliations.length; k++) {
      makeAffButton(affiliations[k], affiliationsLowerAll[k], false);
    }
    updateAffButtons();
