...
L.Polyline.include({
	...
	buildVectorHats: function (options) {
		...
		lineToTrace._parts.forEach((peice, index) => {
			...
			if (options.frequency === 'allvertices') {
			...
			} else if (options.frequency === 'allcentroids') {
				derivedBearings = (() => {
					let bearings = [];
					for (var i = 1; i < latlngs.length; i++) {
						let bearing =
				...
				centroid_sum_x=(latlngs[latlngs.length - 2].lng+latlngs[latlngs.length - 1].lng)
				centroid_x = centroid_sum_x/2
				centroid_sum_y=(latlngs[latlngs.length - 2].lat+latlngs[latlngs.length - 1].lat)
				centroid_y = centroid_sum_y/2
